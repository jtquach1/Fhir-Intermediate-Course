  ...
  new Promise((resolve, reject) => {
    ...
    if (iden) {
      ...
      GetPersonsByIdentifier(
        ...
        GetPatients(person, include, criteria, context, coun, page).then(
          (result) => {
            resolve(result);
          }
        );
...
function PersonToPatientMapper(MyPerson) {
  ...
  if (MyPerson) {
    ...
    if (MyPerson.PRSN_SECOND_NAME != "") {
      ...
      R.name[0].text = R.name.text =
        MyPerson.PRSN_FIRST_NAME +
        " " +
        MyPerson.PRSN_SECOND_NAME +
        " " +
        MyPerson.PRSN_LAST_NAME;
    }
